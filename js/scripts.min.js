...
l.length;a++)l[a].setMap(null);l=[]}function v(a){var b=c.getZoom(),d=Math.abs(a-b),e=a>b?1:-1;for(a=0;a<d;a++)setTimeout(function(){b+=e;c.setZoom(b)},50*(a+1))}function e(b){r.geocode({latLng:b},function(b,c){c==google.maps.GeocoderStatus.OK&&b[1]&&a.each(b[1].address_components,function(b,c){if("locality"==c.types[0])return a("#result-name").text(c.long_name),!1})})}n=new google.maps.DirectionsRenderer({suppressMarkers:!0});r=new google.maps.Geocoder;p=new google.maps.Polyline({strokeColor:"#00796B",
